      ...
      <table style="border-collapse:collapse;font-size:12px;width:100%">
        ...
        ${d.integration ? `<tr><td style="padding:2px 8px;color:#e6edf3">Full Test Suite</td>
            ${d.integration.passed != null
              ...
              : cell(d.integration.mode || '—')}
            ${d.integration.score != null
              ? cell(d.integration.score, undefined, v => (v*100).toFixed(0)+'%')
              : passCell(d.integration.status)}</tr>` : ''}
        <tr><td style="padding:2px 8px;color:#e6edf3">Code Quality (pylint)</td>
            ${cell(d.code_quality?.pylint, undefined, v => v + '/10')}
            ${cell(d.code_quality?.status, undefined, v => v)}</tr>
